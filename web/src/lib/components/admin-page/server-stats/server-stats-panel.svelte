...
<div class="flex flex-col gap-5">
  <div>
    <p class="text-sm dark:text-immich-dark-fg">{$_('components.admin.stats.panel.total')}</p>
      ...
      <div class="flex flex-col justify-between rounded-3xl bg-immich-gray p-5 dark:bg-immich-dark-gray">
        <div class="flex flex-wrap gap-x-12">
          <div class="flex place-items-center gap-4 text-immich-primary dark:text-immich-dark-primary">
            ...
            <p>{$_('components.admin.stats.panel.photos')}</p>
          </div>
        ...
        <div class="flex flex-wrap gap-x-12">
          <div class="flex place-items-center gap-4 text-immich-primary dark:text-immich-dark-primary">
            ...
            <p>{$_('components.admin.stats.panel.videos')}</p>
          </div>
        ...
        <div class="flex flex-wrap gap-x-7">
          <div class="flex place-items-center gap-4 text-immich-primary dark:text-immich-dark-primary">
            ...
            <p>{$_('components.admin.stats.panel.storage')}</p>
          </div>
  ...
  <div>
    <p class="text-sm dark:text-immich-dark-fg">{$_('components.admin.stats.panel.detail')}</p>
    <table class="mt-5 w-full text-left">
      <thead
        class="mb-4 flex h-12 w-full rounded-md border bg-gray-50 text-immich-primary dark:border-immich-dark-gray dark:bg-immich-dark-gray dark:text-immich-dark-primary"
      >
        <tr class="flex w-full place-items-center">
          <th class="w-1/4 text-center text-sm font-medium">{$_('components.admin.stats.panel.detail-user')}</th>
          <th class="w-1/4 text-center text-sm font-medium">{$_('components.admin.stats.panel.detail-photos')}</th>
          <th class="w-1/4 text-center text-sm font-medium">{$_('components.admin.stats.panel.detail-videos')}</th>
          <th class="w-1/4 text-center text-sm font-medium">{$_('components.admin.stats.panel.detail-usage')}</th>
        </tr>
      </thead>
      <tbody
          ...
          >
            ...
            <td class="w-1/4 text-ellipsis px-2 text-sm">
              ...
              <span class="text-immich-primary dark:text-immich-dark-primary">
                ...
                {:else}
                  ({$_('components.admin.stats.panel.detail-unlimited')})
                {/if}
              </span>
            </td>
          </tr>
